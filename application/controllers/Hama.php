...
class Hama extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['data'] = $this->db->query("SELECT * FROM hama_penyakit WHERE status = '1' ORDER BY no_hama_penyakit ASC")->result_array();
        $this->load->view('templates/header');
        $this->load->view('templates/sidebar', $data);
        $this->load->view('hama/hama', $data);
    ...
    public function pertanyaan($id)
    {
        ...
        $this->load->view('templates/header');
        $this->load->view('templates/sidebar', $data);
        $this->load->view('hama/pertanyaan', $data);
        $this->load->view('templates/footer');
    }
        ...
        if ($update) {
            ...
            if ($ubahfoto) {
                ...
                if ($this->upload->do_upload('foto')) {
                    $hama = $this->db->get_where('hama_penyakit', ['id_hama_penyakit' => $id])->row_array();
                    $fotolama = $hama['foto'];
                    if ($fotolama) {
                        unlink(FCPATH . '/uploads/hama/' . $fotolama);
                    }
    ...
    public function detail($id)
    {
        ...
        if ($detail > 0) {
            ...
            $this->load->view('templates/header');
            $this->load->view('templates/sidebar', $data);
            $this->load->view('hama/detail', $data);
            $this->load->view('templates/footer');
        } else {
    ...
    {
        $data['user'] = $this->session->userdata();
        $data['id'] = $this->db->query("SELECT MAX(no_hama_penyakit) as id FROM hama_penyakit WHERE status = '1'")->row_array();
        $this->load->view('templates/header');
        $this->load->view('templates/sidebar', $data);
        $this->load->view('hama/tambah', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function tambahdetail()
    {
        ...
        if ($this->model->insert('detail', $arr)) {
            ...
            redirect('Hama/detail/' . $id_hama_penyakit);
        } else {
            $this->session->set_flashdata(
                'message',
                '<div class="alert alert-danger mb-3" role="alert">Gagal tambah data!</div>'
